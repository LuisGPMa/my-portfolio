...
int main(int argc, char** argv)
{
	...
	//Seleção das sementes na estrutura de sementes
    int qteSementes;
    printf("Digite a quantidade de sementes desejada: ");
    scanf("%i", &qteSementes);
	Semente sem[qteSementes];
	for(int i=0; i<qteSementes; i++){
        sem[i].x = rand() % width;
        sem[i].y = rand() % height;
        sem[i].pxl = in[sem[i].y][sem[i].x];
    }

    //Linha comentada que fornece saída textual de todas as sementes
    //for(int i=0; i<qteSementes; i++){printf("%f %f %u %u %u \n", (float)sem[i].x/width, (float)sem[i].y/height, sem[i].pxl.r, sem[i].pxl.g, sem[i].pxl.b);//}
    ...
    for(int y=0; y<height; y++){
        for(int x=0; x<width; x++){
            mDist = sem[0].pxl;
            distAtual = sqrt(pow(x-sem[0].x, 2) + pow(y-sem[0].y, 2));
            for(int i=1; i<qteSementes; i++){
                if(sqrt(pow(x-sem[i].x, 2) + pow(y-sem[i].y, 2)) < distAtual){
                    distAtual = sqrt(pow(x-sem[i].x, 2) + pow(y-sem[i].y, 2));
                    mDist = sem[i].pxl;
                }
            }
            out[y][x] = mDist;
